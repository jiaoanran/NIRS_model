...
'''
input:wls(wavenum series),xdata(absorbance data),num(the number of samples)
output:draw the spectrum and return plt
'''
    for i in range(num):
...
#draw spectrums that after data preprocessing	
...
def main():
    ...
    xdata = oct_df.iloc[:,8:]
    xdata.columns = wls
    #print(np.array([ int(float(i)) for i in oct_df.iloc[:,7:8].values]))
    xdata.index = np.array([ int(float(i)) for i in oct_df.iloc[:,7:8].values])
